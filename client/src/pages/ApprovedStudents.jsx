...
export default function ApprovedStudents() {
  ...
  if (loading) return <AdminLayout links={adminLinks}><p>Loading...</p></AdminLayout>;

  const renderTable = (list) => (
    <table className="student-table">
      <thead>
        <tr>
          <th>LRN</th><th>Name</th><th>Gender</th><th>Year Level</th><th>Strand</th><th>Type</th><th>Remaining Balance</th><th>Action</th>
        </tr>
      </thead>
      <tbody>
        {list.map((s) => (
          <tr key={`${s.type}-${s.id}`}>
            ...
            <td>
              <Link to={`/admin/queue?type=${s.type}&id=${s.id}`} className="view-link">View Queue Number</Link>{" | "}
              <a href="#" className="view-link" onClick={(e) => { e.preventDefault(); openBalanceModal(s.lrn); }}>View Balance</a>{" | "}
              <Link
    ...
    <AdminLayout links={adminLinks}>
      <div className="approved-students container">
        ...
        <div className="tab">
          ...
          <button className={activeTab === "year" ? "active" : ""} onClick={() => setActiveTab("year")}>By Year Level</button>
          <button className={activeTab === "strand" ? "active" : ""} onClick={() => setActiveTab("strand")}>By Strand</button>
        </div>
        ...
        {activeTab === "year" && (
          Object.keys(byYearLevel).sort((a, b) => a - b).map((yr) => (
            <div className="year-level-section" key={yr}>
              <div className="year-level-header"><h3>Grade {yr}</h3></div>
        ...
        {activeTab === "strand" && (
          Object.keys(byStrand).sort(strandSort).map((strand) => (
            <div className="year-level-section" key={strand}>
              <div className="year-level-header"><h3>{strand}</h3></div>
